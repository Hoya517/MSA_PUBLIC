...
kubectl -n "${NAMESPACE}" set image deployment/discovery-deployment discovery="${IMAGE}"

echo "[discovery] replica를 1로 올려 실행"
kubectl -n "${NAMESPACE}" scale deployment/discovery-deployment --replicas=1

echo "[discovery] 롤아웃 상태 확인"
kubectl -n "${NAMESPACE}" rollout status deployment/discovery-deployment
